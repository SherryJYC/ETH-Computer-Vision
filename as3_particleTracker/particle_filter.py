...
class Simulator(object):
	...
	def moveParticles(self, particles, uk):
		...
		motion = np.random.normal(uk, self.param.odometry_stdev, len(particles))

		for i in range(len(particles)):
			prior_particles[i] = particles[i] + motion[i]
			if self.param.circular_hallway:
	...
	def resampleParticles(self, prior_particles, particle_weights):
		...
		# TODO: Change this value for the correct one
		# normalize weights
		weights_norm = particle_weights / sum(particle_weights)
		# get cumulative weights
		weights_cum = weights_norm
		for i in range(len(weights_cum)):
			if i==0:
				continue
			else:
				weights_cum[i] = weights_cum[i-1] + weights_cum[i]

		# generate uniform u
		us = np.random.uniform(0, 1, len(prior_particles))

		# resampling
		new_index = np.searchsorted(weights_cum, us)
		resampled_particles = [prior_particles[i] for i in new_index]
