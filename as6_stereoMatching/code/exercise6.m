...
% to get a good guess
diff = abs(x1s - x2s);
max_diff =  ceil(max(diff, [], 'all'));

buffer_edge = ceil(max_diff/3);
dispRange = -(buffer_edge+max_diff):(buffer_edge+max_diff);

%% try these fixed ranges
dispRange = -40:40;
%%
